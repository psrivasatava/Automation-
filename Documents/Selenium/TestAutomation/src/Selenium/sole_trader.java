...
WebDriver driver;
	...
	customerType.selectByVisibleText("Sole Trader");
	...
	Select licenseType = new Select(driver.findElement(By.cssSelector("select[data-hook='.licenceType']")));
	licenseType.selectByVisibleText("DVLA Full");
	driver.findElement(By.cssSelector("input[data-hook='.licenceNumber']")).sendKeys("23333333");
	...
	driver.findElement(By.cssSelector("input[data-hook='.numberOfDependants']")).sendKeys("2");

	driver.findElement(By.cssSelector("input[data-hook='.tradeName']")).sendKeys("AUtoma");
	driver.findElement(By.cssSelector("input[data-hook='.establishedYear']")).sendKeys("5");
	driver.findElement(By.cssSelector("input[data-hook='.monthsEstablished']")).sendKeys("5");
	Select businesstype = new Select(driver.findElement(By.cssSelector("select[data-hook='.businessTypeCode']")));
	businesstype.selectByVisibleText("CA Mining & Quarrying Energy Materials ");
	Select businesssubtype = new Select(driver.findElement(By.cssSelector("select[data-hook='.businessSubTypeCode']")));
	businesssubtype.selectByVisibleText("Mining and agglomeration of lignite");
	
	driver.findElement(By.cssSelector("input[data-hook='.natureOfBusiness']")).sendKeys("AUtoma");
	driver.findElement(By.cssSelector("input[data-hook='.taxNum']")).sendKeys("1233");
	driver.findElement(By.cssSelector("input[data-hook='.annualIncome']")).sendKeys("34000");
	...
	action3.moveToElement(driver.findElement(By.cssSelector("input[name='mailingAddress.sameAsMainAddress']"))).click().perform();
	Actions action2 = new Actions(driver);
	action2.moveToElement(driver.findElement(By.cssSelector("input[name='businessAddress.sameAsMainAddress']"))).click().perform();
	...
	//Counting No. of Document in Document section:
			...
			for (int i = 0; i < list1.size(); i++) {
				...
				action4.moveToElement(driver.findElement(By.xpath("//section[@data-hook = 'proposing.summary.documents']//div[@class = 'grid']//div[@class = 'unit unit--800--15']//div[@data-hook = 'documents.fordeadtrees']//wcg-select-all/ul/li["+(i+1)+"]//div//input[@ng-model='document.selected']"))).click().perform();
		...
		for (String child : allWindows1) {
				if (!parent1.equalsIgnoreCase(child)) {
					...
					driver.switchTo().window(child);
						robot.setAutoDelay(500);
					Actions action4 = new Actions(driver);
				action4.moveToElement(driver.findElement(By.cssSelector("input[name='36']"))).click().perform();
